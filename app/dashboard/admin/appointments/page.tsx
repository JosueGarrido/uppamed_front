...
const AdminAppointmentsPage = () => {
  ...
  const handleCreateAppointment = async (e: React.FormEvent) => {
    ...
    setCreating(true);
    try {
      const dateTime = new Date(`${newAppointment.date}T${newAppointment.time || '09:00'}`);
  ...
  const handleEditAppointment = async (e: React.FormEvent) => {
    ...
    setUpdating(true);
    try {
      const dateTime = new Date(`${editingAppointment.date}T${editingAppointment.time || '09:00'}`);
      const appointmentPayload = {
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className="w-full p-2 border border-gray-200 rounded-md focus:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                    disabled={editLoadingSlots || editAvailableSlots.length === 0}
                  >
                    {editAvailableSlots.length > 0 ? (
                      ...
                      <option value={editingAppointment.time || '09:00'}>
                        {editingAppointment.time || '09:00'}
                      </option>
                    )}
                  </select>
                  {editLoadingSlots && (
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-600 flex items-center">
                    ...
                    {new Date(detailAppointment.date).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-1 gap-4">
